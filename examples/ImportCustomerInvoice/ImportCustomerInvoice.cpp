...
{  
    ...
    TATrs_sTransReference[0] = 'I';
    TATrs_sTransReference[1] = 'N';
    TATrs_sTransReference[2] = 'C';
...
//NOUTLINE
   ...
   sprintf (LineTotalAmt, "%2.2f", iTotalPrice + atof (TSIT_ncVATLL));
   OrderLineId[0] = 0;
   ...
   sprintf (Statement, "INSERT INTO C_INVOICELINE  (C_INVOICELINE_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY, C_ORDERLINE_ID, \
                         ...
                         TSIT_nsQty1,  PriceList, StandardPrice, LimitPrice, TotalPrice, LineTotalAmt, 
                         UOMId, Tax, "Y", TSIT_nsQty1, StandardPrice);
    Line += 10;
    iInvoiceLineSeq++;
    MXSetValue (pmessage1, "Commit", 1, &Commit);
    MXSetValue (pmessage1, "SqlStatement", 1, Statement);
...
void main(int argc, char* argv[])
{
   ...
   if (argc > 2)
   {
    ...
    SelectId ("AR Invoice Indirect",      DocumentId,         DOCUMENT_ID);
    ...
    SelectSeq ("C_Invoice", OrderSeq);
    iInvoiceSeq = atol (OrderSeq);

    SelectSeq ("C_InvoiceLine", OrderLineSeq);
    iInvoiceLineSeq = atol (OrderLineSeq);
