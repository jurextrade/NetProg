...
//INOUT
        ...
        sprintf (Statement, "INSERT INTO C_ORDER  (C_ORDER_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY, \
                             ...
                             "N", LastTransReference,  "CL", "CO", "N", "Y", DocumentId, DocumentId, "", 
                             "Y", PurchasesRepId, "to_date (", SDate, "'mm/dd/yyyy')", "to_date (", SDate, "'mm/dd/yyyy')", "to_date (", SDate, "'mm/dd/yyyy')", BPartnerId, BPartnerLocationId, 
                             CurrencyId, "T", PaymentTermId, "D", "A", "I", "P", 
...
//   {
 ...
 //  }
   ...
   if ((strcmp (TATrs_sTransDescEng, "Cash Purchase") == 0) ||
    ...
    {
        sprintf (Statement, "update M_STORAGE SET QTYORDERED = QTYORDERED + %s WHERE M_PRODUCT_ID = %s AND M_LOCATOR_ID = %s ", TSIT_nsQty1, ProductId, LocationId); 

    }
    else
    { 
        sprintf (Statement, "insert into M_STORAGE (M_PRODUCT_ID, M_LOCATOR_ID, AD_CLIENT_ID, AD_ORG_ID, CREATEDBY, UPDATEDBY,   QTYORDERED) \
...
void main(int argc, char* argv[])
{
   ...
   if (argc > 2)
   {
    ...
    sprintf (Statement,  "SELECT StkItemsTransactions.TATrs_nlTransactionId, StkItemsTransactions.Name, AccTransactionHeader.TATrs_ncTotInv, AccTransactionHeader.TATrs_nlVoucherNbr, vew_TransactionRules.TATrs_sTransDescEng, StkItemsTransactions.TSIT_nsQty1, StkItemsTransactions.TSIT_nsQty2, AccTransactionHeader.TATrs_dOperationDate, AccTransactionHeader.ToName, AccTransactionHeader.TATrs_sTransReference, StkItems.Devise, StkItems.Location,  StkItemsTransactions.TSIT_ncUnitPrice, StkItemsTransactions.TSIT_ncUnitPriceLL, StkItemsTransactions.TSIT_nsDiscount, StkItemsTransactions.TSIT_ncTotalPrice, vew_AccTiersCategory.TAccT_sDesc \
            ...
            WHERE (((vew_TransactionRules.TATrs_sTransDescEng) Like '%s') AND ((AccTransactionHeader.TATrs_dOperationDate) %s) AND ((AccTransactionDetails.TATDt_sDesc ='Purchase') OR (AccTransactionDetails.TATDt_sDesc Like 'Cash Purchase'))) \
    ...
    SelectId ("Purchase Order",  DocumentId,         DOCUMENT_ID);
    ...
    SelectId ("Tony Jureidini",  PurchasesRepId,         USER_ID);
