                       ...
                       void* ProcedurePar, int (*AckProcedure)(void* pack, void* papp), void* AckProcedurePar)
{
    ...
    SHORT swCol,swType,swScale,swNull;
    LONG wColLen;
    ...
    */
    ...
    psqlda = OSMallocSqlda ((int)wColLen);
    ...
    psqlda->ColumnsNumber = wColLen;

    size = 0;
    while(i <= (DWORD)wColLen)
    {
        nRet = SQLDescribeCol( m_hStmt, (WORD)i, svColName,256,&swCol,&swType,&pcbColDef,&swScale,&swNull );

        psqlda->Description[i-1]->Name  =    (char *)malloc (strlen ((char *)svColName));
        strncpy (psqlda->Description[i-1]->Name , (char *)svColName, strlen ((char *)svColName));

        psqlda->Description[i-1]->Type       = (int)swType;
        psqlda->Description[i-1]->ItemLength = (int)strlen ((char*)svColName);

        switch ( swType)
        {
        case SQL_CHAR :     /* CHAR = 1 NCHAR*/
            psqlda->Description[i-1]->Type   = OSCHAR;
            psqlda->Description[i-1]->Length = pcbColDef + 1;
            size += pcbColDef + 1;
            break;
       ...
       case SQL_SMALLINT :  
        ...
        default :
            ...
            size += 301;//pcbColDef + 1;
            break;
        }
        psqlda->Description[i-1]->Scale = swScale;
        i++;
    }
