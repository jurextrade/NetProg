...
/* dans ce fichier il n'y est pas toujours !!! à revoir                       */
...
int SYS_ReplyClassConnection (MXMessage* pmessage, MXCom* pcom, void* appfield)
{
    ...
    if ((strcmp (pcomclass->TableName, "") == 0) ||
            ((XlatTable = XFGetXlatFromName(pcomclass->TableName)) == NULL))
        pcom->XF = NULL;
    else
    {
        ...
        {
            Xlat = (XLAT *)malloc(sizeof(XLAT));
            Xlat->FromTo = pcomclass->FromToTranscod;
            Xlat->Tab = XlatTable;
        }
        pxf->Xlat = Xlat;
        pcom->XF = pxf;
