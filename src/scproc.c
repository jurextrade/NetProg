...
SCPROCRESULT SCDialProc   (SC* psc,  SCAction* thisaction)
{
    DWORD Error = 0;
    SCValue*   Value  = NULL;
    SCValue*   Value1 = NULL;
    ...
    Value1 = SCGetParamValue (psc, thisaction, 2);
    SCASSERTDEFINEDVALUE (Value1);
    SCASSERTORVALUES (Value1, SCSTRING, SCCHAR);


    Value2 = SCGetParamValue (psc, thisaction, 3);
    SCASSERTDEFINEDVALUE (Value2);
    SCASSERTORVALUES (Value2, SCSTRING, SCCHAR);

    Error =  OSRasDial ((char*)Value->Val, (char*)Value1->Val, (char*)Value2->Val);
    if (Error != 0)
    {
        if (OSRasError (Error, Buf, 512) == 0)
            SCPutValue (thisaction->Value, Buf, SCSTRING);
        else
            SCPutValue (thisaction->Value, "Error Ras unknown", SCSTRING);
    }
    else
        SCAffectValue (thisaction->Value, (void *)Error, SCLONG);
...
SCPROCRESULT SCHangUpProc   (SC* psc,  SCAction* thisaction)
{
    DWORD Error = 0;
    char Buf[512];
...
SCPROCRESULT SCDialStatusProc   (SC* psc,  SCAction* thisaction)
{
    DWORD Error = 0;
    char Buf[512];
...
SCPROCRESULT SCSetDialEntryProc   (SC* psc,  SCAction* thisaction)
{
    DWORD Error = 0;
    SCValue*   Value = NULL;
    char Buf[512];
    ...
    Error = OSRasGetConnectHandle ((char*)Value->Val);
    if (Error != 0)
    {
        if (OSRasError (Error, Buf, 512) == 0)
            SCPutValue (thisaction->Value, Buf, SCSTRING);
