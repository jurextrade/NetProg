...
int SYS_ShareReply (MXMessage* pmessage, MXCom* pcom,void* appfield)
{
    ...
    /*  sprintf (Buf, "Execution %s terminée avec code d'erreur : %d", paction->ActionClass->Name, Error);
...
int SYS_LoginReply (MXMessage* pmessage, MXCom* pcom,void* appfield)
{
    ...
    /*  if (Error) sprintf (Buf, "Connexion Refusée : (%s, %s) incorrect.", User, Password);
...
int SCFileReplyCommand (MXMessage* pmessage, MXCom* pcom,void* appfield)
{
    ...
    /*  sprintf (Buf, "Execution %s %s terminée avec code d'erreur : %d", paction->ActionClass->Name, FileName, Error);
      SCTrace (psc, Buf);
     */ 
