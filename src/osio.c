    ...
    *diskmode = '0';
    ...
    if (nbelt >= 3)
    {
        ...
        sprintf(Filename, "%*s %*s %c" ,
                strlen (FileName), FileName, strlen (FileType), FileType, disk);
        strcpy (filename, Filename);
    }
    return 1;
}
...
#endif
    return fread (buffer, 1, (size_t)size, (FILE*)fileid->File);
...
#endif
    return fwrite (buffer, 1, (size_t)size, (FILE*)fileid->File);
