...
int MXSetStringValue (MXMessage* pmessage,  void* Value)
{
    ...
    DWORD needed = gotnow +  strlen (Value) + 1;
...
MXApplicationProcedure* MXFindApplicationProcedure (List* ExecuteList, int (*funct)(MX*, void*), void* appfield)
{
    ...
    if (found) return pexecuteprocedure;
    else return NULL;
}
...
MXComCallBack* MXFindComCallBack (List* ExecuteList, int (*funct)(MXCom*, void*), void* appfield)
{
    int found = 0;
    List* plist = ExecuteList;
    MXComCallBack* pexecuteprocedure;
...
int MXFreeAndMoveComIn (TPClient* pclient, void* par)
{
    ...
    /* on libére les EndingProcedures */
...
int MXWriteBufferContent (MX* pmx, MXMessageContext* pcontext, char* stream,
                          char* attributes, int maxcount)
{
    DWORD buffersize;
    int tocopy;
    CHAR buffertype;
...
int MXReadBufferContent (MX* pmx, MXMessageContext* pcontext, char* fromstream,
                         char* attributes, int maxcount)
{
...
LONG MXReadFileContent (MX* pmx, MXMessageContext* pcontext, char* stream,
...
LONG MXWriteFileContent (MX* pmx, MXMessageContext* pcontext, char* stream,
                         char* attributes, WORD maxcount)
{
    char* fromfilename;
    char* tofilename;
...
int MXAddGeneralCallBack (MX* pmx, char* classname, char* messname,
                          BYTE mode, int (*funct)(MXMessage*, MXCom* pcom,void*), void* appfield)
{
