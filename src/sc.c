...
int SCAffectMessageValueToEntityVar (SCAction* paction, void* app)
{
    ...
    for (i = 3; i <= paction->NbParam; i++)
    {
        ...
        switch (pobject->Type) {
        case MXSHORT :
            SCAffectValue (Value, (void *)((SHORT)MXGetValue (pmessage, pobject->Name, 1)), SCINTEGER);
            break;
        case MXWORD :
            SCAffectValue (Value, (void *)((WORD)MXGetValue (pmessage, pobject->Name, 1)),  SCINTEGER);
            break;
        case MXDWORD :
            SCAffectValue (Value, (void*)((LONG)MXGetValue (pmessage, pobject->Name, 1)),  SCLONG);
            break;
        case MXLONG :
            SCAffectValue (Value, (void*)((DWORD)MXGetValue (pmessage, pobject->Name, 1)),  SCLONG);
            break;
        case MXCHAR :
        case MXBYTE :
